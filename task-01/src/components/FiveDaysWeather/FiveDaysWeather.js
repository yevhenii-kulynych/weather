import React, {Component} from "react";
import './FiveDaysWeater.css';
import Item from "../Item/Item";

class FiveDaysWeather extends Component {
  constructor(props) {
    super(props);
    this.state = {
      fiveDaysWeather: []
    };
    this.openWeatherApiKey = 'c792484ade42380886f51003cfcaf04d';
  }

  componentDidMount() {
    fetch(`https://api.openweathermap.org/data/2.5/forecast?q=Kharkiv,ua&units=metric&mode=json&appid=${this.openWeatherApiKey}`)
      .then(result => result.json())
      .then((data) => {
          this.setState({fiveDaysWeather: [...data.list]})
        },
        // error handler
        (error) => {
          throw new Error(error);
        })
  }

  render() {
    return (
      <div className="five-days-weather">
        <h1 className="title">Weather for 5 days in Kharkiv</h1>
        {
          this.state.fiveDaysWeather.map(i => {
            return (
              <Item
                key={i.dt}
                name={i.name}
                country={i.sys.country}
                temp={i.main.temp}
                feelsLike={i.main.feels_like}
                weather={i.weather[0].main}
                description={i.weather[0].description}
                minTemp={i.main.temp_min}
                maxTemp={i.main.temp_max}
                pressure={i.main.pressure}
                humidity={i.main.humidity}
                windSpeed={i.wind.speed}
                date={i.dt_txt}
                state={true}
              />
            )
          })
        }
      </div>
    )
  }
}

export default FiveDaysWeather;